...
import { createReview } from "@/lib/api"; // Real API function
import { Review } from "@/types"; // Real Review type

interface ReviewFormProps {
  setHasReviewed: (value: boolean) => void;
  listingId: string; // listingId is required for the API call
}
  ...
  >({
    ...
    onSuccess: (newReview) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["listing", newReview.listingId],
      });
      setHasReviewed(true);
      setSubmitted(true); // trigger Thank You UI
    },
    onError: (err) => {
    ...
    <AnimatePresence mode="wait">
      {!submitted ? (
        <motion.div
          ...
          className="mt-12 p-8 bg-gray-800 rounded-3xl shadow-xl border-t-4 border-yellow-400 text-white"
        >
        ...
        >
          ...
          {comment && (
            <p className="mt-4 text-gray-200 max-w-xl">&quot;{comment}&quot;</p>
          )}
        </motion.div>
      )}
    </AnimatePresence>
