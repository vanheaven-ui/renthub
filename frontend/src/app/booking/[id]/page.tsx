// components/BookingDetailsPage.tsx (or wherever your BookingDetailsPage is located)
...
// --- RESPONSIVE STAMPS (unchanged from previous fix, included for completeness) ---

// Responsive COMPLETED Stamp
const CompletedStamp = () => (
  // The outer div positions the stamp over the content
  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50 transition-opacity duration-700">
        {/* Inner div contains the stamp styling and text */}   {" "}
    <div
      className="text-center font-extrabold transform -rotate-12 select-none shadow-2xl tracking-widest bg-white/20 backdrop-blur-sm 
        text-5xl sm:text-7xl md:text-9xl 
        text-blue-600 opacity-20 
        border-4 sm:border-6 md:border-8 border-blue-600 
...
const CanceledStamp = () => (
  ...
  <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50 transition-opacity duration-700">
        {/* Inner div contains the stamp styling and text */}   {" "}
    <div
      className="text-center font-extrabold transform rotate-6 select-none shadow-2xl tracking-widest bg-white/20 backdrop-blur-sm 
        text-5xl sm:text-7xl md:text-9xl 
        text-red-600 opacity-30 
        border-4 sm:border-6 md:border-8 border-red-600 
        rounded-xl sm:rounded-2xl 
        p-3 sm:p-4 md:p-6"
    >
            CANCELED    {" "}
...
const BookingDetailsPage = () => {
  ...
  const {
    ...
    enabled: !!bookingId,
  }); // MODIFIED useEffect: Now only sets initial review state on load.

  useEffect(() => {
    if (booking && booking.renterId) {
      // Only find the review if userReview hasn't been set yet (i.e., not via the mutation success)
      if (!userReview) {
  ...
  }, [booking, userReview]); // Added userReview to dependency array // NEW: Handler to instantly update state after form submission
  ...
  const isDueOrPast = bookingEndDate
    ...
    : false; // Update effective status based on payment and end date
  ...
  const currentStatus = effectiveStatus; // --- Edit/Extend logic (fully type-safe) ---
  ...
  const showEditButton: boolean = isCurrentlyExtendable || isPrePaymentEditable; // --- Mutations ---
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-5xl mx-auto bg-white/80 backdrop-blur-md rounded-3xl shadow-2xl p-8 space-y-6 border border-white/40 animate-fadeIn">
                {/* Conditional rendering of the responsive stamps */}       {" "}
        {isBookingCompleted && <CompletedStamp />}       {" "}
        {isBookingCanceled && <CanceledStamp />}       {" "}
        {/* End of stamp rendering */}       {" "}
        <button
        ...
                {/* Review Section - Wrapped in AnimatePresence */}       {" "}
        {isBookingCompleted && (
          <div className="mt-8">
                       {" "}
            <AnimatePresence mode="wait">
              ...
              {hasReviewed && userReview ? (
                <ReviewSubmittedCard key="submitted" review={userReview} />
              ) : (
                <ReviewForm
                  key="form"
                  onReviewSubmitted={handleReviewSubmitted} // Use the new handler
